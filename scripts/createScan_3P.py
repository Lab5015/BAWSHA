...
def main():
    ...
    print("Saving data...")
    writer.write_resonances(path=args.folder_path, data_names=['freq', 'power', 'phase'], data_pos=[0, 1, 2], file_name='Zoomed_peak', label="")
    ...
    if args.fit is True:    
        ...
        for res_name in reader.get_resonances_list():
            ...
            reso = reader.get_resonance(name=res_name, label='')
            freq = reso['freq']*1e-6
            power = reso['power']
